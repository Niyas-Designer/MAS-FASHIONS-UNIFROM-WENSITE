...
const pages = {
  products: {
    seo: ['Uniform Products & Custom Branding Services | MAS FASHION', 'Explore corporate, school, industrial, healthcare and hospitality uniforms with custom embroidery, printing and bulk manufacturing from MAS FASHION.'],
  ...
  industries: {
    seo: ['Uniform Solutions for Every Industry | MAS FASHION', 'Industry-specific uniforms for corporate, education, manufacturing, healthcare, hospitality, retail and service teams across India and export markets.'],
  ...
  manufacturing: {
    seo: ['Uniform Manufacturing Capabilities in Tiruppur | MAS FASHION', 'Discover MAS FASHION’s end-to-end garment manufacturing process: development, pattern making, sampling, cutting, stitching, branding, quality control and logistics.'],
  ...
  quality: {
    seo: ['Uniform Quality Assurance & Inspection | MAS FASHION', 'Learn how MAS FASHION controls uniform quality through material inspection, fabric testing, cutting, stitching, measurement, finishing, audit and packing checks.'],
  ...
  clients: {
    seo: ['Uniform Manufacturing Partner for Organizations | MAS FASHION', 'MAS FASHION supports businesses, schools, factories, hospitals, hotels and service brands with reliable uniform programs and responsive account support.'],
  ...
  careers: {
    seo: ['Careers at MAS FASHION | Garment Industry Jobs in Tiruppur', 'Explore careers in production, quality, merchandising, design, sales and operations at MAS FASHION in Tiruppur, Tamil Nadu.'],
    eyebrow: 'Careers at MAS FASHION',
    ...
    cta: ['See yourself at MAS FASHION?', 'Send your profile, preferred role and a short introduction. We will contact suitable candidates when an opportunity matches.'],
  },
}
...
function CareersActions({ light = false }) {
  return <div className={`cta-actions ${light ? 'cta-actions-light' : ''}`}>
    <div>
      <a className="btn btn-gold" href="mailto:manzoor@zealgroups.in?subject=Career enquiry at MAS FASHION">Email your profile <Icon name="arrow" size={17}/></a>
...
export default function ContentPage({ page }) {
  ...
  return <motion.main className="content-page" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}>
    ...
    <section className="content-hero">
      ...
      <div className="content-hero-image"><img src={data.image} alt={`${data.eyebrow} at MAS FASHION`} /></div>
    </section>

    <section className="inner-overview container section-pad">
      <Reveal className="inner-overview-copy"><span className="eyebrow">The MAS FASHION approach</span><h2>{data.overview[0]}</h2><p>{data.overview[1]}</p><p>{data.overview[2]}</p></Reveal>
