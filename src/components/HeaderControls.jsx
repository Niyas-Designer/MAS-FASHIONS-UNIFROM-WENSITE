...
const searchItems = [
  ...
  { title: 'About MAS FASHION', type: 'Page', path: '/about', keywords: 'company story strengths tiruppur' },
...
export function SearchControl({ open, onClose }) {
  ...
  return <AnimatePresence>
    {open && <Overlay onClose={onClose} label="Search MAS FASHION" className="search-overlay">
  ...
  return <AnimatePresence>
    {open && <Overlay onClose={onClose} label="Trade account" className="account-overlay">
      <div className="account-visual">
        <div><span className="eyebrow">MAS FASHION trade portal</span><h2>Uniform programs,<br/><em>managed simply.</em></h2><p>Access quotations, order updates, specifications, and repeat-order support in one place.</p></div>
      </div>
      <div className="account-panel">
        ...
        {complete ? <motion.div className="account-success" initial={{opacity:0,y:14}} animate={{opacity:1,y:0}}>
          ...
          <p>{mode === 'signin' ? 'This front-end control is ready to connect to your authentication system.' : 'Our team will contact you to set up your MAS FASHION trade account.'}</p>
